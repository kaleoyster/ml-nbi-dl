...
def preprocess(csv_file = '../data/nebraska_deep.csv'):
    ...
    y = pd.Series(y).map(map_y)
    y_holdout = pd.Series(y_holdout).map(map_y)

    return X, y, (X_holdout, y_holdout), columns_final
